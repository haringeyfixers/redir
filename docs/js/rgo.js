...
let urlParams = {}, hfsw="h", hfp="p"; //CHANGE THIS SWTICH PARAMETER FOR EACH DIFFERENT rd JS FILE
...
const nextAppointment = s22getNearestFutureEvent(hfevents);
/**
 * Finds the nearest future date object from an array of objects.
 * @param {Array} eventData - Array of { date: string|Date, url: string }
 * @returns {Object|null} - The object with the closest future date or null.
 */
function s22getNearestFutureEvent(eventData) {const now = new Date();
  return eventData.reduce((nearest, current) => {
    const currentDate = new Date(current.date);

    if (currentDate > now) {// 1. Check if the date is in the future
      // 2. If we haven't found a future date yet, or if this one is sooner than our current 'nearest'
      if (!nearest || currentDate < new Date(nearest.date)) {return current;}
    }
...
case 'h43'	:ff_targ_text='Haringey Fixers Past Repairs'; ff_targ='https://docs.google.com/spreadsheets/d/1PeSyLKmi4tophgGlAk_ibcVGnw87pKLqYq3bexHNx34/edit?usp=sharing';	break; 
        // https://bit.ly/hfpast https://haringeyfixers.github.io/redir/rdhfix.htm?h=h43	
